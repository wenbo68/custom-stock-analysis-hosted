...
class StockRepository:
    ...
    def get_history_bars(
        self, *, code: str, end_date: date, limit: int
    ) -> List[StockDaily]:
        """Return up to ``limit`` daily bars ending at ``end_date``
        (inclusive), oldest first — e.g. for volatility lookbacks."""
        with self.db.get_session() as session:
            rows = session.execute(
                select(StockDaily)
                .where(and_(StockDaily.code == code, StockDaily.date <= end_date))
                .order_by(desc(StockDaily.date))
                .limit(limit)
            ).scalars().all()

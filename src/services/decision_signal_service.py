...
# Inlined from the parent project's portfolio service (the only thing this
# module used from it). The standalone app has no portfolio feature.
VALID_MARKETS = frozenset({"cn", "hk", "us", "jp", "kr", "tw"})
...
class DecisionSignalService:
    ...
    def _cached_holding_identities(self, *, account_id: Optional[int]) -> set[Tuple[str, str]]:
        # No portfolio feature in the standalone app: nothing is ever "held",
        # so holding-only queries simply match no signals.
        del account_id
        return set()
